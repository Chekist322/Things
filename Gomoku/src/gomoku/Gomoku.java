...
public class Gomoku extends GameSearch {
        ...
        jbtIcon.addActionListener(
            ...
            public void actionPerformed(ActionEvent e) {
                if (!gameState) {
                    ...
                    if(jrbEasy.isSelected()) maxDepth = 1;
                    else if(jrbNormal.isSelected()) maxDepth = 2;
                    else if(jrbHard.isSelected()) maxDepth = 3;
                }
                else {
                    setDefaultStatue();
                    chessboard.repaint();
    ...
    public float positionEvaluation(Position p, boolean player){
        GomokuPosition pol = new GomokuPosition();
    ...
    private int[][] connectN(Position p, boolean player, int number) {
        ...
        for(int n = 2; n <= number; n++) {
            for(int i = 0; i < pos.grid.length; i++) {
                for(int j = 0; j < pos.grid.length; j++) {
                        ...
                        if (mouseX >= xStart && mouseX <= xEnd && mouseY >= yStart && mouseY <= yEnd) {
                            ...
                            if(board.grid[gridX][gridY] == GomokuPosition.BLANK) {
                                ...
                                Thread thread2 = new Thread(new Runnable() {
                                    public void run() {
                                        Vector v = alphaBeta(0, board, PROGRAM);
